...

{
    ...
    //vertexAttribPointer是将属性绑定到当前的ARRAY_BUFFER，也就是positionBuffer,假如现在利用版顶点随意将ARRAY_BUFFER绑到其他数据上，该属性依然从positionBuffer上读取数据
}
{
    let primitiveType = gl.TRIANGLES;//着色器运行的 图元类型
    let offset = 0;//着色器从第几次开始运行
    let count = 3;//着色器运行次数，
