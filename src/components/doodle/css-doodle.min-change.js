(function (global, factory) {
    ...
    class Rules {
      ...
      add_rule(selector, rule, count) {
        let rules = this.rules[selector];
        if (!rules) {
          rules = this.rules[selector] = [];
        }

        if(count != undefined && rules.length == 0){
          for(let p in window.doodleData){
            rules.push(`--${p}:${doodleData[p][count - 1]};`);
          }
        }
      ...
      compose_rule(token, coords, selector) {
        ...
        if (prop == 'content') {
          // let propname = value.match(/\((.*)\)/)[1];
          // console.log(this.rules[this.compose_selector(coords.count)])
          if (!/^(counter\(|val\(|["'])/.test(value)) {
            value = `'${ value }'`;
          }
        }
      ...
      output() {
        Object.keys(this.rules).forEach((selector, i) => {
          if (is_parent_selector(selector)) {
            let globalDataStyle = '';
            for(let p in window.doodleGlobalData){
              globalDataStyle += `--${p}:${window.doodleGlobalData[p]};`
            }
    ...
    class Doodle extends HTMLElement {
      ...
      setListData(name, data){
        doodleData[name] = data;//Array
      }

      setGlobalData(name, value){
        doodleGlobalData[name] = value;
      }

      addCustomStyle(selector){
        let styleNode = document.querySelector(selector);
        styleNode.parentNode.removeChild(styleNode);
        window.doodleCustomStyle = styleNode.innerHTML;
      }
      ...
      update(styles) {
        ...
        if(!this.doodle.querySelector('#customStyle')){
          let customStyle = document.createElement('style');
          customStyle.setAttribute('id', 'customStyle');
          customStyle.innerHTML = window.doodleCustomStyle;
          this.doodle.appendChild(customStyle);
        }
      }
  ...
  window.doodleData = {};
  window.doodleGlobalData = {};
  window.doodleCustomStyle = '';