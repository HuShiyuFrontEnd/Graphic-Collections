...
<script>
    ...
    export default {
        mounted(){
            ...
            var force = 50;//我们约定右为力的正方向
            var K = 5;//虎克定律 L = F / K;
            var stickForce = 100;//与交互施加的力相反方向的粘滞力，超过这个值，将脱离粘滞
            var movingTime = 500;
            var movingDistance = 0;//220为一个单位
            var ps = [0, 0, 0];
            window.inputForce = function(val){
                force = val;
            }
            //将力等参数转化为形变量
            function transformForce(){
                //d/e参数与拉扯距离的关系是 d * 1.1 * r = L
                if(force > stickForce){
                    move(force);
                }if(force > 0)
                    ps[0] = force / K / 33; 
                else ps[1] = force / K / 33; 
            }
            ...
            function anim(){
                ...
                transformForce();
                drawOneframe(ps[0], ps[1], index * 220);
