...
const LINEAR_EASE = function(t, b, c, d){
    if(t > d) t = d;
    return ((c - b) * t / d + b) ;
}
...
function Sprite({width, height, x, y} = {}){
    ...
    this.computeBound = function(){
        ...
        for(let child of this.children){
            let bound = [child.x + child.bound?child.bound[0]:0, child.y + child.bound?child.bound[1]:0, child.width + child.x, child.height + child.y];
        ...
        this.height = max;
        this.width = max;
        this.buffer.width = max;
        this.buffer.height = max;
    ...
    this.render = (compute) => {
        ...
        for(let item of this.children){
            this.tempCtx.drawImage(item.buffer, item.x, item.y);
        }
...
function Scene(width, height){
    ...
    this.render = () => {
        this.ctx.clearRect(0, 0, this.width, this.height)
        for(let item of this.children){
            this.ctx.drawImage(item.buffer, item.x, item.y);
        }
    }
}
...
window.gameObject = new GameObject();
...
let Tween =(function(game){
    return function(){
        ...
        this.to = (setting, time, easing, istransition) => {
            this.animateIn = true;
            this.stack.push({
                type:'to',
                to:setting,
        ...
        this.render = (total) => {
            ...
            for(let key in this.start){
                ...
                if(key == 'x') console.log(this.target[key]) 
            }
            this.target.render();
        }
        ...
        this.next = (total) => {
            ...
            if(next.type == 'wait') setTimeout(() => {
                ...
                this.rendering = true;
                this.easing = next.easing || LINEAR_EASE;
            }
            this.nextTrigger = false;
        }
        this.destory = () => {
        ...
        Subscriber.register('progress',(current, total) => {
            if(current == total){
                ...
                let anim_main = new Tween()
                    ...
                    .to({x:50, y:-270, scaleX:1, scaleY:1},500)
                    .to({x:-50, y:-230, scaleX:1.1, scaleY:1.1},9000)
                ...
                let anim_book = new Tween()
                    .get(this.flyingBook)
                    .from(['rotate','x','y'])
                    .to({rotate:30,x:30,y:30},10000,false,true)
